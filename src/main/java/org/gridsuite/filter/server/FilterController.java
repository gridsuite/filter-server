...
public class FilterController {
    ...
    @GetMapping(value = "/filters/export/hvdcStations", produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "Export a list of hvdc filters to vsc converter station ids in a JSON format")
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "The filters on JSON format")})
    public ResponseEntity<List<FilterEquipments>> exportFiltersVscConverterStations(@RequestParam("ids") List<UUID> ids,
                                                                   @RequestParam(value = "networkUuid") UUID networkUuid,
                                                                   @RequestParam(value = "variantId", required = false) String variantId) {
        List<FilterEquipments> ret = service.exportVscConverterStationsFromHvdcFilters(ids, networkUuid, variantId);
        Logger.getLogger("export").info(() -> String.format("multiple net:%s, variant:%s, ids:%s,%ngot:%d",
                networkUuid, variantId, ids.stream().map(UUID::toString).collect(Collectors.joining()), ret.size()).replaceAll("[$\r]", "_"));
        return ResponseEntity.ok()
                .contentType(MediaType.APPLICATION_JSON)
